...
func schedinit() {
	...
	// In par ticular, it must be done before mallocinit below calls racemapshadow.
...
func mcommoninit(mp *m) {
	...
	// allm 指向 mp，通过 allm 和 mp.alllink 可以找到所有 m
...
func casgstatus(gp *g, oldval, newval uint32) {
	...
	// 如果gp-> atomicstatus处于扫描状态，则循环，使GC有时间完成并将状态更改为oldval
...
func mstart() {
	...
	// 对于无法使用 g0 stack 的系统，直接在系统的堆栈上划出所需空间
	if osStack {
...
func mstart1() {
	...
	// 记录调用方作为mcall调用的堆栈的顶部用来终止线程
	// 在调用调度之后，不会返回 mstart1,所以调用者可以重用当前帧
...
func allocm(_p_ *p, fn func()) *m {
	...
	// 只是单纯的释放 free m 列表, 和 allocm 没啥关系，就是每次在新分配一个 m 的时候，清理全局空闲 m 列表，并把对应的 g0 空间释放掉
	if sched.freem != nil {
		lock(&sched.lock)
		var newList *m
...
func handoffp(_p_ *p) {
	...
	// p 的运行队列不为空或者调度器全局队列不为空
	if !runqempty(_p_) || sched.runqsize != 0 {
		// 找一个新的 m 执行这个 p, 线程启动的时候会先调用 mstart
		startm(_p_, false)
		return
	}
	...
	if atomic.Load(&sched.nmspinning)+atomic.Load(&sched.npidle) == 0 && atomic.Cas(&sched.nmspinning, 0, 1) { // TODO: fast atomic
		// p 运行队列为空，新建或者找一个自旋 m， 把 m 和 p 绑定
		startm(_p_, true)
		return
	}
	lock(&sched.lock)
	...
	if _p_.runSafePointFn != 0 && atomic.Cas(&_p_.runSafePointFn, 1, 0) {
	 ...
	 // 又判断一遍 全局队列是不是空，应该和上面的 GC 有关，上面唤醒之后需要再判断一次吧
	if sched.runqsize != 0 {
		unlock(&sched.lock)
		startm(_p_, false)
		return
	...
	// 最后一个在运行的 p，其他的 p 都处在空闲状态并且调度器没有 m 轮训
	...
	// 一直到最后都没有 m 就放到空闲 p 列表
	// 也就是说
	// 1. 没有等待运行的 g
	// 2. 存在自旋的 m 或者 闲置的 p （说明资源过剩）
	pidleput(_p_)
	unlock(&sched.lock)
}
...
func stoplockedm() {
	...
	// m 没有 lockedg 或者 lockedg 的 m 指向的不是当前 g 的 m
	...
	if _g_.m.p != 0 {
		...
		// 删除 p 和 m 的关系
		_p_ := releasep()
		// 把 p 交出去，交给别的 m 或者放到空闲队列里
		handoffp(_p_)
	}

	// 增加已锁住的 m 数量，并检查有没有死锁
	incidlelocked(1)
	// Wait until another thread schedules lockedg again.
	// 等待直到另外的线程调度 lockedg， 此处是被阻塞了
	notesleep(&_g_.m.park)
	// 被唤醒之后，清除标识
	noteclear(&_g_.m.park)
...
//
// 调度 gp 运行在当前的 m， gp 继续当前的时间片，否则，重新开始一个新的时间片
// 如果 inheritTime 是 true，
//go:yeswritebarrierrec
func execute(gp *g, inheritTime bool) {
	...
	if !inheritTime {
		// 增加调度次数，每次调度都增加，可以用来和 sysmon 一起轻量级判断是否抢占
...
top:
	...
	if sched.runqsize != 0 {
		lock(&sched.lock)
		// 虽然 max 为 0，但是是批量拿全局队列里的 g，第一个 g 返回，其他的 g 都放到 p 里
	...
	// anyway.
	// 从网络任务里拿
	// epoll 已经初始化，并且有等待的套接字，而且当前不在轮训状态
	...
	// 当前 m 不处在自旋状态并且总自旋的 m 数 *2 大于忙碌的 p ，跳到 stop
	// 这对于防止过度的CPU消耗, 当GOMAXPROCS >> 1但程序并行度较低时很有必要。
	if !_g_.m.spinning && 2*atomic.Load(&sched.nmspinning) >= procs-atomic.Load(&sched.npidle) {
		goto stop
	}
	// 上面找了自己的 p， 找了全局的，又找了 netpoll 里的，而且还不是自旋的 m 也不是太多，咱就自旋把，从别的 p 里偷
	// 把当前 m 改成自旋
	if !_g_.m.spinning {
		_g_.m.spinning = true
	...
	// 尝试 4 次
	for i := 0; i < 4; i++ {
		// 在 初始化 allp 的时候初始化的 stealOrder, 随机器随机出来的 p 的索引
		for enum := stealOrder.start(fastrand()); !enum.done(); enum.next() {
			...
			if i > 2 && shouldStealTimers(p2) {
				...
				if ran {
					...
					// is a local G to run.
					// 检查并且运行 timers 可能会产生新的 g，这个时候就要偷一偷了
					if gp, inheritTime := runqget(_p_); gp != nil {
						return gp, inheritTime
					}
					ranTimer = true
	...
	// 尝试了 4 次，不管检查的是别的 p 的 runnext、runq 队列，还是 timers，都没有能偷的 g， 那就 stop 吧
	if ranTimer {
...
stop:
	...
	for _, _p_ := range allpSnapshot {
		// 突然发现有 g 的 p 了，赶紧麻溜的变成自旋状态，然后再去跳到偷循环里
		// 吐槽一句，go 官方不建议用 goto, 但是自己用的挺溜啊
		if !runqempty(_p_) {
			lock(&sched.lock)
			_p_ = pidleget()
			unlock(&sched.lock)
			...
			// 前面已经和 p 解绑了，这次虽然有非空队列的 p，但是没有空闲的 p，就出去吧
			break
		}
	}
...
// the when argument.
// 唤醒网络轮询器中休眠的线程， 如果有一个，并且无论如何都不会在when参数之前唤醒
...
func schedule() {
	...
	// 存在锁住的 g
	// 一开始不可能锁住，因为一开始执行的是 main
	if _g_.m.lockedg != 0 {
		// 把 p 交出去，并且锁住 m，直到被唤醒，再把 nextp 和 m 绑定
		stoplockedm()
		// 被唤醒之后就执行 lockedg , 不会返回(唤醒之前可能把 g 分配给了一个 p)
...
top:
	...
	// 如果 m 在自旋，队列应该为空
	// 调用 checkTimers 之前应该检查这个，否则可能会调用 goready 把一个 ready 的 g 放进本地运行队列
	...
	// 因为偷到 g 了，没偷到 g 就会被阻塞了，到不了这里。 把 m 从自旋的状态中改回来
	if _g_.m.spinning {
		resetspinning()
	}
	...
	if gp.lockedm != 0 {
		...
		// 把 p 交给 gp.lockedm ，并且把当前的 m 阻塞住
		startlockedm(gp)
		goto top
	}
...
func goexit0(gp *g) {
	...
	if locked {
		...
		// the thread.
		// goroutine可能已锁定此线程，因为它使它处于异常的内核状态。 杀死它，而不是将其返回到线程池。
		// 返回 mstart 将会释放 p 和退出这个线程
		// 不太明白啊
		if GOOS != "plan9" { // See golang.org/issue/22227.
			...
			// this thread.
			// 清除plan9上的lockedExt，因为我们可能最终会重用此线程
			_g_.m.lockedExt = 0
		}
	}
	schedule()
...
func newproc1(fn *funcval, argp unsafe.Pointer, narg int32, callergp *g, callerpc uintptr) {
	...
	newg.sched.sp = sp
	newg.stktopsp = sp
	// 重点来了， 这里保存的是 goexit 的地址
	newg.sched.pc = funcPC(goexit) + sys.PCQuantum // +PCQuantum so that previous instruction is in same function
	newg.sched.g = guintptr(unsafe.Pointer(newg))

	// 重点来了2， 此处调用是关键,这里把 goexit 入栈，也就是说把 sp  指向 newg.sched.pc
	// 在 gostartcall 中把 newproc1 时设置到 buf.pc 中的 goexit 的函数地址放到了 goroutine 的栈顶，然后重新设置 buf.pc 为 goroutine 函数的位置。
	// 这样做的目的是为了在执行完任何 goroutine 的函数时，通过 RET 指令，都能从栈顶把 sp 保存的 goexit 的指令 pop 到 pc 寄存器，
	// 效果相当于任何 goroutine 执行函数执行完之后，都会去执行 runtime.goexit，完成一些清理工作后再进入 schedule。
	...
	// 放到 p 的队列里， p 队列满了就放到全局队列里, 此处是先放到 runnext
	runqput(_p_, newg, true)

	// 当前有空闲的 p 并且当前没有 m 自旋等待并且主 m 已经启动了
	// 初始化的时候，除了被 m0 绑定的 p，其他的 p 都是空闲 ps
...
func gfpurge(_p_ *p) {
	// gfree 和 g.schedlink 构成了空闲 g 列表
	lock(&sched.gFree.lock)
	for !_p_.gFree.empty() {
		gp := _p_.gFree.pop()
		_p_.gFree.n--
...
func (pp *p) destroy() {
	...
	if len(pp.timers) > 0 {
		...
		// 锁了两个 p 的 timersLock, 因为要把 timer 从 pp 转移到 plocal
...
func procresize(nprocs int32) *p {
	...
	if _g_.m.p != 0 && _g_.m.p.ptr().id < nprocs {
		...
		// 继续使用这个 p， 假如 m0 存在 p， 那么 pp.init 就不会把 p0.mcache 和 m0 绑定
		// 设置 p 为运行状态
	...
	for i := nprocs - 1; i >= 0; i-- {
		...
		// 把其他未使用的 p 放进空闲队列里去
		p.status = _Pidle
		// 运行队列里不存在 g 的 p 才算空闲 p
		if runqempty(p) {
			// 把 p 放到调度器空闲 p 的队列里
			pidleput(p)
		} else {
			// 从调度器空闲 m 队列中找一个 m 给 p
			p.m.set(mget())
			// 可运行的 p 以链表形式相连
...
//
//go:yeswritebarrierrec
// 关联 p 和当前 m 以及 mcache，并把 p 的状态设置为运行状态
func acquirep(_p_ *p) {
...
func checkdead() {
	...
	// 如果我们由于在一个已经闲置的线程上捕获到信号而垂死，Frozentheworld将导致所有正在运行的线程阻塞。 运行时实质上将进入死锁状态，除了有一个线程即将调用exit之外
	if panicking > 0 {
		return
	}
	...
	// for details.)
	// 如果我们不是在 cgo 下运行，但是有一个额外的 m 我们需要考虑这个 m
	var run0 int32
	...
	// 上面已经判断过了不存在运行中的 m, 所以下面肯定不存在运行中的 g
	grunning := 0
	lock(&allglock)
	...
	if oldTimers {
		...
		// 找到 timers 的 p，绑定到 m 上，执行。 因为没有运行中的 m 了，说明没有可运行的 g 和 p 了，那就从 timers 中一个 p，再找一个空闲的 m 去运行
		_p_ := timejump()
		if _p_ != nil {
...
type sysmontick struct {
	schedtick   uint32 // 观察时的调度次数
	schedwhen   int64 // 观察时的时间点
	syscalltick uint32 // 观察时系统调用的次数
	syscallwhen int64 // 观察时系统调用的时间
}
...
func retake(now int64) uint32 {
	...
	for i := 0; i < len(allp); i++ {
		...
		if s == _Prunning || s == _Psyscall {
			...
			// 被 sysmon 观察时候的 schedtick 不等于现在的 schedtick，说明从上一次观察到现在，调度了不同的 g
			if int64(pd.schedtick) != t {
				pd.schedtick = uint32(t)
				pd.schedwhen = now
			} else if pd.schedwhen+forcePreemptNS <= now {
				// 从上次被 sysmon 观察的时间到现在超过 10 ms
				// 执行抢占， 发出抢占信号，此时还没有真正的抢占
				preemptone(_p_)
		...
		if s == _Psyscall {
			...
			// sysretake 为 false, 和上次观察时系统调用的时钟不一样
			...
			// 当前 p 的本地队列是空的，并且存在自旋 m 或者空闲的 p and 执行系统调用的时间不超过 10 ms
...
func preemptone(_p_ *p) bool {
	...
	// 在 goroutine 中每一个调用都会进行栈溢出检测，比较当前堆栈指针和 gp->stackguard0
	// 将gp-> stackguard0设置为StackPreempt会将抢占折叠到正常的堆栈溢出检查中
	// 因为 stackPreempt 非常小，所以堆栈检查的时候会扩容，调用 runtime.morestack  asm.amd64.s
	// runtime.morestack 会调用 runtime.newstack
...
// Sched must be locked.
// 批量从全局队列里拿 g，返回第一个 g，并把其他的 g 放到 p的本地队列中去
...
// 偷一半，要么就偷一个 runnext(因为被偷的 p 也就只有一个 runnext 了)
...
func (q *gQueue) pushBackAll(q2 gQueue) {
	...
	if q.tail != 0 {
		// q 的最后一个 g 的 schedlink 执行 q2 的 head
		q.tail.ptr().schedlink = q2.head
	} else {
		q.head = q2.head
	}
	// 因为是通过 g 的 schedlink 来进行链表，所以老的 q.tail 并没有丢，所以新的 q.tail 被赋值为 q2.tail
	q.tail = q2.tail
}
...
// 初始化 allp（procresize 这个方法） 的时候初始化这个 p 的随机器
...
// 该算法基于以下事实：如果我们有X，使得X和GOMAXPROCS是互质的，则（i + X）％GOMAXPROCS的序列将提供所需的枚举
